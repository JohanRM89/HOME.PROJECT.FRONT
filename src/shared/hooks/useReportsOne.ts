...
import { ReposneGetDataReport } from "@/modules/members/domain/IMembersRepository";
import { MemeberApi } from "@/modules/members/infrastructure/memeber.api";
import { useFocusEffect } from "expo-router";
import { useCallback, useState } from "react";

export function useReportsOne(idTask?: string | null) {
  const useCase = new MemberCase(new MemeberApi());
  const [reports, setReports] = useState<ReposneGetDataReport>();
  ...
  const loadTasks = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await useCase.GetReportsOne(idTask);
      setReports(response);
    } catch (err) {
